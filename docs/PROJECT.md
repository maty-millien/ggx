...
Codex uses `gpt-5.6-luna` with no reasoning effort. To skip the CLI's multi-second startup, ggx sends the prompt straight to the Codex backend (`chatgpt.com/backend-api/codex/responses`) through `curl`, authenticating with the access token and account id that the Codex CLI stores in `$CODEX_HOME/auth.json` (`~/.codex/auth.json` by default) and collecting the streamed text deltas. If that file is missing or the request fails, for example because the token has expired, ggx falls back to `codex exec`, which refreshes and persists the token so the next call takes the fast path again.

Claude uses `claude-haiku-4-5` (the CLI's `haiku` alias) with no reasoning effort: direct API requests send `output_config.effort: none`, while the CLI fallback passes no effort flag because it rejects `none`. It follows the same direct-first pattern: ggx posts the prompt to the Anthropic Messages API (`$ANTHROPIC_BASE_URL/v1/messages`, defaulting to `api.anthropic.com`) through `curl` with the credentials the Claude CLI itself uses. It prefers `ANTHROPIC_AUTH_TOKEN` (as a bearer token) or `ANTHROPIC_API_KEY` from the environment; otherwise it reads the CLI's OAuth login from `$CLAUDE_CONFIG_DIR/.credentials.json` (`~/.claude/.credentials.json` by default) or, on macOS, from the `Claude Code-credentials` Keychain item via `security`, which macOS may ask to allow once. OAuth requests carry the `oauth-2025-04-20` beta header and a system prompt that opens with the Claude CLI's identity line, which the API requires for those tokens. When credentials are missing, expired, or the request fails, ggx falls back to `claude --print`, which refreshes and persists the OAuth token so the next call takes the fast path again.

Each command requests all of its generated artifacts together. ggx removes one surrounding JSON markdown fence from any provider before parsing and validation.
