...
Codex uses `gpt-5.6-luna` with no reasoning effort. To skip the CLI's multi-second startup, ggx sends the prompt straight to the Codex backend (`chatgpt.com/backend-api/codex/responses`) through `curl`, authenticating with the access token and account id that the Codex CLI stores in `$CODEX_HOME/auth.json` (`~/.codex/auth.json` by default) and collecting the streamed text deltas. ggx never falls back to the Codex CLI: if that file is missing, the token is rejected, or the request fails, the command stops with an error that tells the user to run `codex login`.

Claude uses `claude-haiku-4-5`, which does not accept an effort parameter, so none is sent. It follows the same direct-only pattern: ggx posts the prompt to the Anthropic Messages API (`$ANTHROPIC_BASE_URL/v1/messages`, defaulting to `api.anthropic.com`) through `curl` with the credentials the Claude CLI itself uses. A proxy configured for the CLI through `ANTHROPIC_BASE_URL` and `ANTHROPIC_AUTH_TOKEN` (sent as a bearer token) therefore routes ggx too. Anthropic API keys are not supported; otherwise it reads the CLI's OAuth login from `$CLAUDE_CONFIG_DIR/.credentials.json` (`~/.claude/.credentials.json` by default) or, on macOS, from the `Claude Code-credentials` Keychain item via `security`, which macOS may ask to allow once. OAuth requests carry the `oauth-2025-04-20` beta header and a system prompt that opens with the Claude CLI's identity line, which the API requires for those tokens. ggx never falls back to the Claude CLI: when credentials are missing, expired, or the request fails, the command stops with an error that tells the user to run `claude login`.
