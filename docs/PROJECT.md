...
3. Generate a short branch name using OpenCode CLI with the model selected by `GGX_MODEL`, or OpenCode Zen model `opencode/north-mini-code-free` by default, with variant `none`.
...
ggx uses OpenCode CLI for branch names, commit messages, and PR copy. Set `GGX_MODEL` to any model available in the user's OpenCode configuration; when it is unset or empty, ggx falls back to OpenCode Zen model `opencode/north-mini-code-free`. The OpenCode variant remains `none`.
